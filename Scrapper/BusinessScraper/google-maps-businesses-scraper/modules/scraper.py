...
def scrape(args):
    ...
    for place in SETTINGS["PLACES"]:
        ...
        print("{fore.GREEN}Moving on to {place}{fore.RESET}")
        ...
        for _ in range(0, SETTINGS["PAGE_DEPTH"]):
            ...
            for box in boxes:
                ...
                if scraped and args.skip_duplicate_addresses:
                    print("{fore.WARNING}Skipping {name} as duplicate by address{fore.RESET}")
                else:
                    ...
                    if scraped:
                        ...
                        print("{fore.WARNING}Currently scraping on{fore.RESET}: {name}, for the {addresses_scraped[address]}. time")
                    else:
                        addresses_scraped[address] = 1
                        print("{fore.GREEN}Currently scraping on{fore.RESET}: {name}")
            ...
            try:
                ...
                print("{fore.WARNING}No more pages for this search. Advancing to next one.{fore.RESET}")
    ...
    print("{fore.GREEN}Done. Time it took was {elapsed}s{fore.RESET}")